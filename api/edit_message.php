...
if (!in_array($scope, ['public', 'private', 'group'], true) || $messageId <= 0) {
...
// در گروه، ویرایش پیام دیگران با ادمین/مالک همان گروه است، نه ادمین کل سایت
...
$result = edit_message_body($scope, $messageId, $body);
if ($result === null) {
    http_response_code(404);
    echo json_encode(['error' => 'not_found']);
    exit;
...
echo json_encode([
    ...
    'edited_time_label' => date('H:i', strtotime($result['edited_at'])),
