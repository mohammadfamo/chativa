...
$groupId = (int) ($data['conversation_id'] ?? ($_POST['conversation_id'] ?? 0));
...
// اگر پیامِ ریپلای‌شده معتبر (و در همین گروه) نباشد، بی‌صدا نادیده گرفته می‌شود
$replyToId = (int) ($data['reply_to_id'] ?? ($_POST['reply_to_id'] ?? 0));
if ($replyToId > 0) {
    $checkStmt = db()->prepare('SELECT id FROM group_messages WHERE id = ? AND group_id = ? AND deleted_at IS NULL');
    $checkStmt->execute([$replyToId, $groupId]);
    if ($checkStmt->fetchColumn() === false) {
        $replyToId = 0;
    }
}

$stmt = db()->prepare('INSERT INTO group_messages (group_id, sender_id, body, reply_to_id) VALUES (?, ?, ?, ?)');
$stmt->execute([$groupId, $me['id'], $body, $replyToId > 0 ? $replyToId : null]);
...
$messageData = [
    ...
    'body'         => $body,
    'time_label'   => date('H:i'),
    'reply_to'     => $replyToId > 0 ? get_reply_info('group', $replyToId) : null,
    'user_id'      => (int) $me['id'],
...
broadcast('group:' . $groupId, 'message', $messageData);
